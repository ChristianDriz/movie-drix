...
export default function TrendingSection({ type, title, initialMediaLists } : Props ) {
    ...
    return (
        <section className="px-2 py-4 md:px-4 max-w-7xl mx-auto w-full relative">
            <div className="flex items-center gap-4">
                <h2 className="font-semibold text-lg xl:text-xl capitalize">{title}</h2>
                {/* Filter by date range */}
                <div className="font-light flex items-center justify-evenly outline outline-white/10 rounded-full bg-midnight text-white/80 text-xs xl:text-sm relative">
                    <button onClick={() => setDuration("day")} className="z-10 px-2.5 py-1 xl:px-3 rounded-full cursor-pointer ">
                        Today
                    </button>
                    <div className={`absolute h-full w-1/2 rounded-full bg-blue transition-all ${duration === 'day' ? '-translate-x-[27px] xl:-translate-x-[32px]' : 'translate-x-[27px] xl:translate-x-[32px]'}  `}></div>
